...
text = lambda x: (
    ...
    else re.sub(r"\d+", "", re.sub(r"[^\w\s]", "", x.lower())).strip()
)
...
train_reviews, test_reviews, train_ratings, test_ratings = train_test_split(
    df["content"].values, df["score"].values, test_size=0.2, random_state=42
)
# 자연어 처리
...
train_dataset = ReviewDataset(
    train_reviews, train_ratings, text_pipeline, label_pipeline
)
...
for epoch in range(num_epochs):
    ...
    print(
        f"Epoch {epoch+1}/{num_epochs}, Average Loss: {total_loss/len(train_dataloader):.4f}"
    )  # 에포크와 손실 출력
